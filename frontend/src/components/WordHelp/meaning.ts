/**
 * Parses the lightly-marked-up "meaning" strings from the TBESG lexicon into a
 * tree of {@link MeaningSpan}s — a small, presentation-agnostic model that
 * `MeaningText` turns into React components. The source markup is HTML-ish but
 * not valid HTML (e.g. `<ref='Act.20.37'>`, `<re>`, `__1.` sense markers), so
 * it is parsed leniently with htmlparser2 rather than fed to the DOM.
 *
 * Also derives the deduped set of meanings worth showing for a word
 * ({@link meaningsOf}).
 */
...
/** TBESG tags that denote a line break and never wrap content. */
...
/**
 * TBESG `<ref='Mat.1.1'>` tags omit both the space and the attribute name that
...
/**
 * Parses TBESG meaning markup into a {@link MeaningSpan} tree. htmlparser2 runs
...
/** The distinct meanings worth showing for a word, deduped. */

...
// a plain map, knowing nothing about database/sql. Converting that map into a
// specific driver's argument shape is the consumer's job — this is the SQLite
// consumer's converter, living in the persistence layer rather than in the
// domain ref package. database/sql wants a slice of sql.Named, so the conversion
// is a loop rather than a cast.
//

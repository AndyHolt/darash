"""Load parsed TAHOT Words (and their Segments) into Postgres.

Two tables: ``tahot_words`` (one row per Hebrew word, carrying variant metadata
and frequency stats) and ``tahot_word_segments`` (one row per morpheme /
punctuation mark, carrying its Strong's number and decoded morphology). The
segment's ``word_id`` references a word ``id`` assigned in Python before the COPY
so the two streams stay linked without round-tripping through a SERIAL sequence.

DROP+CREATE+COPY runs inside a single transaction in load_words(), so a mid-load
failure rolls back to the previous ingest's data (Postgres DDL is transactional).
"""
...
CREATE TABLE tahot_words (
    ...
    has_meaning_variant BOOLEAN NOT NULL,
...
def load_words(conn: psycopg.Connection, words: list[Word]) -> None:
    """Recreate the tables and bulk-load all words and segments in one txn.
    ...
    with conn.transaction():
        ...
        with conn.cursor().copy(
            f"COPY tahot_words ({_WORD_COLUMNS}) FROM STDIN"
        ) as copy:
            for w in words:
        ...
        segment_count = 0
        with conn.cursor().copy(
            f"COPY tahot_word_segments ({_SEGMENT_COLUMNS}) FROM STDIN"
        ) as copy:
            for w in words:
                for s in w.segments:
                    seg_id += 1
                    copy.write_row(_segment_to_row(seg_id, w.id, s))
                    segment_count += 1

...
How Darash reaches production, and the one-time setup behind it.

## Topology

- **Frontend** → built with Vite and synced to an S3 bucket behind a CloudFront
  distribution.
- **Backend** → a Docker image (with `data.sqlite` baked in) pushed to ECR and
  run as a Lambda behind a Function URL, via the AWS Lambda Web Adapter.
- **`/api/*`** requests hit the *same* CloudFront distribution, which signs them
  (OAC) and forwards to the Lambda Function URL — so the frontend always calls
  same-origin `/api/...` and there is no CORS to manage.

No RDS/ECS/ALB (to keep costs down). DNS for `darashbible.com` lives in
Cloudflare (a grey-cloud CNAME to CloudFront), not Route53.

Infrastructure is Terraform, split into two root modules:

- [`infra/bootstrap/`](../infra/bootstrap) — applied locally once; creates the
  Terraform state bucket, the GitHub OIDC provider, and the CI role.
- [`infra/app/`](../infra/app) — the application infrastructure, applied by CI
  (`infra-deploy.yml`).
...
separate Terraform configuration under
[`infra/bootstrap/`](../infra/bootstrap). That module is applied locally rather
than via CI (it's what grants CI its permissions in the first place). See
...
AWS console or CLI. Validate it using DNS (add the CNAME record in Cloudflare).
...
backend Lambda's Function URL, avoiding CORS issues. After the first
`infra/app/` apply that creates the CloudFront distribution:
...
Point the function at an earlier image. Each deploy pushes an image tagged with
its commit SHA, so pick a previous SHA (the function's Terraform ignores
`image_uri`, so this won't be reverted by `infra-deploy`):
```bash

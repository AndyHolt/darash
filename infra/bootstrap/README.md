...
These resources can't be managed from `infra/app/` because `infra/app/` uses
the S3 bucket as its state backend, and the `terraform-ci` role is what runs
it. So
...
creates is what stores remote state for `infra/app/`, so it can't hold its own
...
Day-to-day infra changes go through `infra/app/` and the `infra-deploy.yml`
workflow, not here.
